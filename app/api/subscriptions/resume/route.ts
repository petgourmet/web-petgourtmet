/**
 * API Route: Reanudar Suscripción
 * 
 * POST /api/subscriptions/resume
 * 
 * Reactiva una suscripción pausada en Stripe y actualiza el estado en la base de datos
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Reanudar en Stripe (quitar la pausa)
    const resumedSubscription = await stripe.subscriptions.update(
      subscription.stripe_subscription_id,
      {
        pause_collection: null as any, // Remover la pausa
      }
    )

    // Calcular próxima fecha de pago
    const nextBillingDate = resumedSubscription.current_period_end 
    ...
    const { data: updated, error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        current_period_start: resumedSubscription.current_period_start 
          ? new Date(resumedSubscription.current_period_start * 1000).toISOString()
          : null,
    ...
    try {
      ...
      // Email al cliente
      await sendSubscriptionEmail('resumed', {
