-- Migración para optimizar las consultas de productos - Versión 2
-- Fecha: 2025-01-27
...
-- Índice para categorías activas (con productos)
CREATE INDEX IF NOT EXISTS idx_categories_with_products 
ON categories (id, name) 
WHERE id IN (SELECT DISTINCT category_id FROM products WHERE stock > 0);
