...
export function AuthForm({ defaultMode = "login" }: { defaultMode?: "login" | "register" } = {}) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // El navegador redirige automáticamente a Google — no hay más código aquí
    } catch (error: any) {
    ...
    <div className="w-full max-w-md mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
      ...
      <div className="p-6 hover:scale-105 transition-transform duration-300 cursor-pointer">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 border border-gray-300 dark:border-gray-600 rounded-md py-2 px-4 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {googleLoading ? (
